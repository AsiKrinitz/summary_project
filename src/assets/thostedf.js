var TzlaHostedFields = new (function () {
  var userfCallbId, userTerminal_name;
  ...
  var hufo = function (userConfig) {
        ...
        function (msgEventObj) {
          ...
          try {
            ...
            if (err instanceof SyntaxError) {
              console.log(
                "Ignore: Message from space received: " + err.message
              );
            } else {
              throw err;
            }
    ...
    function triggerChargeReceived(rcvdMsg) {
      evntCallBacks["charge." + rcvdMsg.requestId] &&
        evntCallBacks["charge." + rcvdMsg.requestId].forEach(function (
          fCallBack
        ) {
    ...
    function triggerChallenge(rcvdMsg) {
      ...
      //    var strIframe='';
      //    var resultObj={};
      //   // alert (rcvdMsg.response.challengeUrl);
      //    strIframe = "<iframe src='"+rcvdMsg.response.challengeUrl+ "' width='500' height='800'></iframe>";
      //    challengeDiv.innerHTML=strIframe;
      ...
      //Set styles
      div.style.cssText =
        "position:fixed;transform:translate(-50%, -50%);z-index:1000000;background-color:transparent;top:50%;left:50%;width:410px;height:650px;border:none";
      iframe.style.cssText = "width:400px;height:650px;border:none";
      span.style.cssText =
        "color:#aaaaaa;float:right;font-size:28px;font-weight:bold;cursor:pointer;";
      ...
      // span.addEventListener("click", () => document.getElementById("hf-challenge-cont").remove());
    ...
    function containerValidityCSSClass(rcvdMsg) {
      var elem = document.querySelector(
        userConfig.fields[rcvdMsg.fieldName].selector
      );
      ...
      if (elem) {
        if (elem.className)
          elem.className.includes &&
            !elem.className.includes(clsName) &&
            (elem.className += " " + clsName);
        else elem.className = clsName;
      }
    }
    ...
    function generateAllHFIframes(userCfgFields, userCfgStyles) {
      Object.keys(userCfgFields).forEach(function (cfgFldId) {
        ...
        insertHFIFrameAndPumpUp(
          userCfgFields[cfgFldId].selector,
          cfgFldId,
          userCfgFields[cfgFldId],
          function (ifElem, cfgFldObj) {
            var stylemsg = {
              type: "applyStyles",
              data: userCfgStyles,
            };
    ...
    function makeHFFocusOnLabelClick(ifElem, cfgFldObj) {
      ...
      labelElem.forEach &&
        labelElem.forEach(function (lblElem) {
    ...
    function insertControlIFrameAndPumpUp(onloadCallBack) {
      ...
      ctrlIframe.src =
        tzlaHFBaseURL + "/controlfields.html?instance_identifier=" + instanceID;
    ...
    function isDMYHostedField(fname, cfgFldObj) {
      var ret = false;
      if (cfgFldObj.version && cfgFldObj.version === "2") ret = true;

      return ret;
    }

    function insertHFIFrameAndPumpUp(
      cfgFldSel,
      cfgFldId,
      cfgFldObj,
      onloadCallBack
      ...
      else addCSSClassToElem(containerElem, "container-for-hostedfield");
    ...
    function createHFSrc4Iframe(fname, cfgFldObj) {
      ...
      if (isDMYHostedField(fname, cfgFldObj))
        ...
        src =
          tzlaHFBaseURL +
          "/genfield.php?field_name=" +
          fname +
          "&instance_identifier=" +
          instanceID;
        if (cfgFldObj.placeholder)
          src += "&placeholder=" + cfgFldObj.placeholder;
        if (cfgFldObj.label) src += "&label=" + cfgFldObj.label;
      }

      if (autCompl) src += "&autocomplete=" + autCompl;
    ...
    function checkUserConfig(ucfg) {
      ...
      chkObjElementsNO(
        Object.keys(ucfg.fields),
        1,
        "Config fields must have at least 1 key, " + hfIDs
      );
      hfIDs.forEach(function (id) {
        if (hfOptionalIDs[id] && !ucfg.fields[id]) {
          ...
          chkObjProp(
            ucfg.fields[id],
            "selector",
            "Config fields." + id + ".selector must be present"
          );
          chkType(
            ucfg.fields[id].selector,
            "string",
            "Config fields." + id + ".selector must be a string"
          );
          var allElements4OneHF = document.querySelectorAll(
            ucfg.fields[id].selector
          );
          chkObjElementsNO(
            allElements4OneHF,
            ...
            "Selector for field '" +
              id +
              "' matches " +
              allElements4OneHF.length +
              " elements, must match 1 element"
          );

          ucfg.fields[id].labelSelector &&
            chkType(
              ucfg.fields[id].labelSelector,
              "string",
              "options.fields." + id + ".labelSelector must be a string"
            );
        }
      });
    }
    ...
    function chkObjElementsNO(obj, num, err) {
      if (obj.length < num) throw err;
    }

    function chkObjProp(obj, prop, err) {
      if (!obj[prop]) throw err;
    }

    function chkType(obj, type, err) {
      if (typeof obj !== type) throw err;
    }

    function chkIfObj(obj, err) {
      if (!obj || obj.constructor !== Object) throw err;
    }

    function generateInstanceID() {
      return (
        genFromStr(Math.floor(268435456 * (1 + Math.random()))) +
        genFromStr(genFromDT() + 65536)
      );
    }
    ...
    var tzlaCtrlIframe,
      ...
      iframeCSS =
        "border: none; width: 100%; height: 100%; float: left; overflow: hidden;",
      sandBox = false,
    ...
    objMyself.trigger = function (trigMsg, trigMsgExtra) {
      ...
      evntCallBacks[trigMsg.type] &&
        evntCallBacks[trigMsg.type].forEach(function (evntCallBack) {
